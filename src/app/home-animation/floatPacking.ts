...
export function encodeFloatRGBA_16(value, scale) {
    value = value * scale + 2;
    var pair = [
        Math.floor((value % 2) / 2 * 255),
        Math.floor(Math.floor(value / 2) / 2 * 255)
    ];
    return pair;
}

export function decodeFloatRGBA_16(pair, scale) {
    return (((pair[0] / 255) * 2 +
             (pair[1] / 255) * 2 * 2) - 2) / scale;
}
